function loss = solve_model_calibration(m,param)
    % given an m (the slope), it calcultates p_f=m*rank_f+constant. Solves the model with
    % these probabilities and return the loss for calibration
    ...
    amin=param.amin;
    da=param.da;
    disutility_working=param.disutility_working;
    dw=param.dw;
    firing_rate=param.firing_rate;
    firm_rank=param.firm_rank;
    firm_size_share=param.firm_size_share;
    ga=param.ga;
    ...
    % employed
    constant = 1/N_f*(1-m*sum(firm_rank));
    p = m*firm_rank'+constant;
    ...
    sum_g = sum(g_e,"all")*da*dw+sum(g_u)*da; %normalizing so that the integral is equal to one
    g_u=g_u/sum_g;
    g_e=g_e/sum_g;
